...
- `POST /api/v1/b2b/service-centers/{id}/scan` — скан автопарка из CRM → ретеншн-дела (идемпотентно).
- Ядро рекомендаций — то же (`recommender.Advisor`). Включается при `APP_SECRET_KEY`.

**Отложено:** OAuth/Marketplace, авторизация b2b-эндпоинтов, периодический шедулер, обратная синхронизация
